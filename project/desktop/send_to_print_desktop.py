...
# user_states = {}
# order_codes = {}
# order_numbers = set()  # Множество для хранения использованных номеров заказов
# order_number = 1  # Инициализация номера заказа
#
# # Инициализация блокировок
# order_number_lock = threading.Lock()
# document_processing_lock = threading.Lock()  # Блокировка для обработки документов
#
# class BotThread(QThread):
#     message_received = pyqtSignal(str)
#
...
#         @bot.message_handler(content_types=['document'])
#         def handle_document(message):
#             global order_number  # Объявляем переменную как глобальную
#             try:
#                 if not self.ready_to_receive_files:
#                     bot.reply_to(message, "Сначала нажмите /new_order, чтобы начать прием файлов.")
#                     return
...
#                 code_order = random.randint(10000, 99999)  # Генерация 5-значного числа для check_number
#                 while code_order in order_numbers:  # Проверка на уникальность
#                     code_order = random.randint(10000, 99999)
#
#                 final_file_name = f"order_{order_number}{os.path.splitext(message.document.file_name)[1]}"
...
#                     new_file.write(downloaded_file)
#
#                 # Отправка сообщения о том, что заказ обрабатывается
#                 processing_message = bot.send_message(message.chat.id, "Ваш заказ обрабатывается, пожалуйста, подождите...")
#
#                 # Блокировка для обработки документов
#                 with document_processing_lock:
#                     # Подсчет страниц
#                     pages = None
#                     if final_file_name.lower().endswith('.pdf'):
#                         pages = get_pdf_page_count(final_file_path)
#                     elif final_file_name.lower().endswith(('.docx', '.doc')):
#                         pages = get_docx_page_count(final_file_path)
#
...
#                     'check_number': code_order,
#                     'pages': pages,
#                     'extension': os.path.splitext(final_file_name)[1],
#                 }
#
#                 # Добавление заказа в order_codes
#                 with order_number_lock:
#                     order_codes[order_number] = order_data
#                     order_numbers.add(code_order)  # Добавляем номер заказа в множество
#
#                 # Запрос цвета печати с клавиатурой
#                 markup = telebot.types.ReplyKeyboardMarkup(
#                     one_time_keyboard=True,
#                     resize_keyboard=True
#                 )
#                 markup.row('Черно-белая', 'Цветная')
#
#                 bot.send_message(message.chat.id, "Выберите цвет печати:", reply_markup=markup)
#                 bot.register_next_step_handler(message, lambda msg: process_color_selection(msg, order_data))
#
#                 order_number += 1  # Увеличиваем номер заказа после обработки
#
#             except Exception as e:
#                 logging.error(f"Document handler error: {e}")
#                 bot.reply_to(message, "Произошла ошибка при обработке файла")
...
#                     bot.send_message(
#                         message.chat.id,
#                         f"Заказ №{order_number - 1} принят!\n"  # Используем order_number - 1 для отображения
#                         f"Код для получения: {order_data['check_number']}",
#                         reply_markup=remove_keyboard
#                     )
#                     self.message_received.emit(order_data['file_path'])
